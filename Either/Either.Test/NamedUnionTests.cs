...
public class NamedUnionTests
{
    ...
    [InlineData(SwitchType.Stmt)]
    [InlineData(SwitchType.Expr)]
    public async Task Switch_With_A_Named_Union_Type_And_Conditional_Access_And_No_Null_Case_Complains(SwitchType switchType)
    {
        var code = Shared.GenerateSwitchForANamedUnion(switchType, "IntOrBool", ["int", "bool"], new() { TagSwitch = true, IsUnionTypeNullable = true });
        var expected = VerifyCS.Diagnostic(EitherAnalyzer.NotExhaustiveId)
            .WithLocation(0)
    ...
    [InlineData(SwitchType.Stmt)]
    [InlineData(SwitchType.Expr)]
    public async Task Switch_With_A_Named_Union_Type_And_Conditional_Access_And_Null_Case_Succeeds(SwitchType switchType)
    {
        var code = Shared.GenerateSwitchForANamedUnion(switchType, "IntOrBool", ["int", "bool", "null"], new() { TagSwitch = true, IsUnionTypeNullable = true });
        await VerifyCS.VerifyAnalyzerAsync(code);
    }
}
